...
def test_large_saved_history_is_streamed_and_measured(monitoring_profile):
    ...
    with closing(sqlite3.connect(db_path)) as db, db:
        ...
        global_plan = [
            row[3] for row in db.execute("EXPLAIN QUERY PLAN " + global_sql, global_args)
        ]
        target_plan = [
...
"""
    ...
    print(
        json.dumps(
            {"measurements": measurements, "global_plan": global_plan, "target_plan": target_plan},
            sort_keys=True,
        )
    )
...
def test_oversized_multibyte_row_is_projected_out_before_large_scan(monitoring_profile):
    ...
    page = run(
        profile,
        "snapshots.targets",
        validate_params("snapshots.targets", {"username": "alice"}),
