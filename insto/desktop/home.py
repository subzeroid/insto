...
def _config_report(path: Path) -> tuple[str, str | None, Config | None]:
    ...
    backend = config.backend if config.backend in _BACKENDS else None
    if backend != BACKEND_HIKERAPI:
        return "ok", backend, config
    token = config.hiker_token
    if not isinstance(token, str):
        return "invalid", backend, None
    try:
        ...
        with own.locked(initialize=True, verify_binding=False):
            ...
            if current is not None:
                ...
                if same_own or same_adopted:
                    config = _own_config(current, deadline) if not current.adopted else None
                    if current.adopted:
                        _, config = await _inspect(current.home, deadline=deadline)
            ...
            if path is not None:
                ...
                # Everything that can refuse happens before the own service is stopped:
                # the target's state/journal/backup must be readable and settled, and
                # its service directories creatable.
                require_settled(target)
                target.read_state()
                _prepare_directories(target)
                adoption = (report, config)
            if adoption is not None and (current is None or not current.adopted):
            ...
            # The new home's state is durable before the binding points at it: a crash
            # in between leaves a stale state file, never a bound home without intent.
