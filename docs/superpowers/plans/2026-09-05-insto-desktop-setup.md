...
**Completed locally.** Verification, native evidence, artifact hashes and remaining gates are recorded in [C1 results](2026-09-05-insto-desktop-setup-results.md). No publication or merge.
...
- `setup.configure` and `credentials.replace` accept exactly `{"token": "..."}`; other operations accept an empty object. Tokens contain 4–4096 ASCII characters in the range 33–126, with no whitespace or controls. The minimum matches the secret redactor's registration floor. No profile paths, backend choice, runtime path or arbitrary native arguments come from JSON.
...
For an empty first-time profile, publish journal, validated config and bound state under the profile lock before constructing the service lease (which creates controller directories). Existing-profile credential changes retain both locks and executor exclusion. Atomic file stages live in the equally private application root, outside `profile/`; interrupted stages are retained and never treated as committed config or recovery authority. A backup is published by one rename under the exclusive profile lock, after refusing an existing target, avoiding a crash-stranded intermediate hardlink.
...
If a nonterminal recovery journal exists, explicit Stop refuses `recovery_required` before any controller/native work instead of starting a service as a side effect of recovery. Repair must finish recovery first. Repair with a pending record performs reconciliation only and reports the observed state; explicit Start is a separate subsequent action. Terminal-record cleanup never itself starts the service.

Native proof confirmed the transitional states `xpcproxy`, `SIGTERMed`, and the exit sentinel `(never exited)`. They are not readiness. A `kickstart` client timeout is an uncertain native result: after the started worker drains, retain the same lease and observe exact owned readiness within the remaining operation/readiness budget. Never retry kickstart automatically, add `-k`, or increase its 10-second native cap. Other native errors, cancellation and bootstrap timeouts still fail. Recorded native command timing distinguishes successful observations from an assumed successful command reply.
...
- [x] Red: `.venv/bin/python -m pytest tests/test_desktop_access.py -q` must fail for the missing access boundary.
- [x] Implement `DesktopError(code)` and a static catalog. `validate_candidate(token: str) -> int` registers the secret before lazy SDK import, calls a desktop-local `make_backend(token)` directly constructing HikerBackend (never the environment-sensitive backend registry), uses the strict C0 `validate_access`, and closes once in `finally`. The SDK adapter initializes BaseClient with its explicit packaged default host and creates its HTTP transport with `trust_env=False`, so poisoned provider/proxy/CA environment cannot affect construction or requests. Preserve CLI defaults. Invalid params fail before construction; no console logging or exception reflection. Access errors map to `invalid_token`, `quota_exhausted`, `rate_limited`, `network_error`, `access_unconfirmed`, `operation_timeout`; filesystem/lifecycle errors have their own codes.
- [x] Green, Ruff and mypy; commit only access/errors and tests. Independent spec review, then code-quality review.

## Task 2: Private owned profile storage

- [x] Add filesystem tests for no-effect missing reads, unsafe roots/components/files, ownership binding, 64KiB read bounds, state schema/types, atomic replacement, directory fsync, stable nonblocking lock contention and no secret in state.
...
- [x] Wire lazy operation imports, keep `hello` SDK/config/database-free and advertise exactly the C1 list. Update C0 capability assertions without weakening transport tests. Wrap only known safe `DesktopError` codes; unexpected details become static `internal_error`. No production fake operation or credential echo.
- [x] Add isolated installed-process tests for `hello`, missing setup inspection, invalid params and pending recovery reporting. A repeated read must leave the managed root unchanged and import no provider SDK.
- [x] Green desktop/full regression tests, commit and two-stage review.

## Task 6: Installed/native verification and handoff

- [x] Run `.venv/bin/python -m pytest --cov=insto --cov-report=term-missing -q`, `.venv/bin/ruff check .`, `.venv/bin/ruff format --check .`, `.venv/bin/mypy insto`, and `git diff --check`. Repeat service/desktop tests with `python -B`.
- [x] Build wheel/sdist using frozen hash-bearing build constraints into a new temporary evidence directory; install wheel non-editably into a separate venv. Verify package origin, new hello capabilities and missing-profile inspect with `-I -B`, cleaned environment and no real token.
- [x] Native fixture exercises the new controller from the installed wheel with fake config only: start, idempotence, explicit stop, restart after loaded clean exit, matching executor readiness, cleanup. A test-only Python fixture can inject boundaries; no fake knob is added to the production protocol. Reuse the previously reviewed owned-process-group supervisor/cleanup rules; run unit failure-injection before native state changes. Do not claim live HikerAPI validation from offline tests.
- [x] Update `docs/desktop-protocol.md`, README if appropriate, and add `2026-09-05-insto-desktop-setup-results.md` with exact tests, artifacts, remaining P1/C2/G1/G2/R1 gates and any unproved native credential scenario. Keep the old C0/P0 historical results intact.
- [x] Final independent Astra whole-change review, address verified findings with red/green regressions, rerun checks, commit and preserve local branch. No push, merge, signing or release. Verify no `Co-authored-by:` trailer in any commit.
